...
class TutorialViewModel {
  ...
  func fetchTutorial() {
    ...
    let videoURL = URL(string: "https://api.jsonbin.io/b/5ed67c667741ef56a566a831")!
    let videoPublisher = URLSession.shared.dataTaskPublisher(for: videoURL)
      .map(\.data)
      .decode(type: APIResult.self, decoder: decoder)
    
    let articleURL = URL(string: "https://api.jsonbin.io/b/5ed679357741ef56a566a67f")!
  ...
  private func updateTutorials() {
    ...
    tutorials.sort { $0.attributes.releasedAt > $1.attributes.releasedAt }
    DispatchQueue.main.async {self.tutorials = .success(tutorials)}
  }
